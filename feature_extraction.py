...
COUNTRY_CAPITAL_LAT_LON   = [ "Afghanistan Kabul !!!34.28.00+ 69.11.00+ ",
...
def location_cluster_nearest_capital(dataset, data_point):
  ...
  point to the capital is closest to by Haversine distance. Acts as
  an approximator for country of origin given Google API call limit.
  '''
  global COUNTRY_CAPITAL_LAT_LON
  global IS_CREATED_CAPITAL_DICT
  global CAPITAL_DICT
  ...
  def country_capital_lat_lon_dict(raw_data):
    '''
    Create a mapping from each country/capital string to
    its corresponding longitude and latitude decimal coordinates.
    '''
    ccll_dict = {}
    for ccll in raw_data:
      cc = ccll.split('!!!')[0]
      cc = string.strip(cc)
      ...
      la_sign = la[-1]
      lo_sign = lo[-1]
      latitud = la[0:-1].split('.')
      longitu = lo[0:-1].split('.')
      la_deci = float(latitud[0]) + float(float(latitud[1])/float(60))
      lo_deci = float(longitu[0]) + float(float(longitu[1])/float(60))

      if la_sign == '-':
        la_deci = float(-1) * la_deci
      if lo_sign == '-':
        lo_deci = float(-1) * lo_deci
      ccll_dict[cc] = (la_deci,lo_deci)
    return ccll_dict
  ...
  country_vector = [0] * len(alphabetical_country_list)
  pnt_lat, pnt_lon = data_point['location_latitude'], data_point['location_longitude']
  
  shortest_country   = alphabetical_country_list[0]
  shortest_haversine = float("inf")
  for country in alphabetical_country_list:
    cap_lat, cap_lon = CAPITAL_DICT[country][0], CAPITAL_DICT[country][1]
    haversine_dist   = haversine(pnt_lat, pnt_lon, cap_lat, cap_lon)
    if haversine_dist < shortest_haversine:
      shortest_country   = country
      shortest_haversine = haversine_dist
  country_vector[alphabetical_country_list.index(shortest_country)] = 1
...
def apply_machine_learning_algorithm(x_dataset, y_dataset, ml_func=linear_model.LinearRegression(), split_proportion=0.20, permute=False, k_folds=5, graph_pca=False, pca_preprocess=False, pca_reduction=0):
  '''
  ...
  split_point = int(len(x_dat)*split_proportion)

  if graph_pca:
    graph_pca_data(x_dat[0:5000], y_dat[0:5000])

  if pca_preprocess:
    exact_dimension = len(x_dat[0])
    x_dat = PCA(n_components=exact_dimension).fit_transform(x_dat)

  if pca_reduction > 0:
    if pca_reduction > len(x_dat[0]):
      x_dat = PCA(n_components=len(x_dat[0])).fit_transform(x_dat)
    else:
      x_dat = PCA(n_components=pca_reduction).fit_transform(x_dat)
  ...
  X_train = np.array(x_dat[split_point:-1])
  Y_train = np.array(y_dat[split_point:-1])

  X_test  = np.array(x_dat[0:split_point])
  Y_test  = np.array(y_dat[0:split_point])
